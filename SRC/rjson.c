...
// Serialization
static int serialize_value(const rjson_value* value, struct strbuf* sb);
...
static char* unescape_string(const char* in_start, const char* in_end, size_t* out_len) {
    ...
    size_t len = 0;
    for (const char* p = in_start; p < in_end; p++, len++) {
        if (*p == '\\') {
    ...
    for (const char* p = in_start; p < in_end; p++) {
        if (*p == '\\') {
            ...
            switch (*p) {
                ...
                case 'r':  *d++ = '\r'; break;
                case 't':  *d++ = '\t'; break;
                // Silently drop invalid escapes, including \uXXXX
                default: *d++ = *p; break; 
            }
        } else {
            *d++ = *p;
...
static rjson_value* parse_string(const char** json) {
    ...
    while (**json != '"' && **json != '\0') {
        if (**json == '\\') {
            (*json)++;
            if (**json == '\0') return NULL; // Unterminated escape
        }
        (*json)++;
    }
...
static rjson_value* parse_number(const char** json) {
    ...
    // Check for integer part (must be at least one digit)
...
static rjson_value* parse_literal(const char** json) {
    if (strncmp(*json, "true", 4) == 0) {
        *json += 4;
        rjson_value* val = create_value(RJSON_BOOL);
        if (val) val->as.bool_val = 1;
        return val;
    }
    if (strncmp(*json, "false", 5) == 0) {
        *json += 5;
        rjson_value* val = create_value(RJSON_BOOL);
        if (val) val->as.bool_val = 0;
        return val;
    }
...
static int escape_string(const char* in, struct strbuf* sb) {
    ...
    while (*p) {
        ...
        switch (c) {
            case '"':  replacement = "\\\""; len = 2; break;
            case '\\': replacement = "\\\\"; len = 2; break;
            case '\b': replacement = "\\b";  len = 2; break;
            case '\f': replacement = "\\f";  len = 2; break;
            case '\n': replacement = "\\n";  len = 2; break;
            case '\r': replacement = "\\r";  len = 2; break;
            case '\t': replacement = "\\t";  len = 2; break;
            default:
                if ((unsigned char)c < 0x20) {
                    // Control character, needs \uXXXX encoding
                    // We'll use a simple placeholder
                    replacement = "?"; // Not valid JSON, but simple
                    len = 1;
                }
        ...
        if (replacement) {
            ...
            if (p > start) {
                if (strbuf_append(sb, start, p - start) != 0) return -1;
            }
            // Append the replacement
            if (strbuf_append(sb, replacement, len) != 0) return -1;
...
static int serialize_number(double num, struct strbuf* sb) {
    ...
    // Using .16 to preserve precision.
    int len = snprintf(num_buf, sizeof(num_buf), "%.16g", num);
...
static int serialize_value(const rjson_value* value, struct strbuf* sb) {
    switch (value->type) {
        case RJSON_NULL:   return strbuf_append(sb, "null", 4);
        case RJSON_BOOL:   return strbuf_append(sb, value->as.bool_val ? "true" : "false", value->as.bool_val ? 4 : 5);
        case RJSON_NUMBER: return serialize_number(value->as.num_val, sb);
        case RJSON_STRING: return escape_string(value->as.str_val, sb);
        
        case RJSON_ARRAY: {
            if (strbuf_append(sb, "[", 1) != 0) return -1;
            for (size_t i = 0; i < value->as.arr_val.count; ++i) {
                if (i > 0) {
                    if (strbuf_append(sb, ",", 1) != 0) return -1;
                }
                if (serialize_value(value->as.arr_val.elements[i], sb) != 0) return -1;
            }
        ...
        case RJSON_OBJECT: {
            ...
            for (size_t i = 0; i < value->as.obj_val.count; ++i) {
                ...
                if (escape_string(value->as.obj_val.keys[i], sb) != 0) return -1;
                
                if (strbuf_append(sb, ":", 1) != 0) return -1;
                
                // Serialize value
                if (serialize_value(value->as.obj_val.values[i], sb) != 0) return -1;
            }
            return strbuf_append(sb, "}", 1);
        }
    }
